...
const style = {
  ...
  byuButton: `bg-[#1A1F3A] text-[#6188FF] p-1 px-3 text-sm rounded-lg cursor-pointer hover:opacity-20`,
}
...
const CoinNameRow = ({ name, icon, clicked }) => {
  ...
  return (
    <div className={style.coinNameRow}>
      <div className="mr-3 flex" onClick={clicked}>
        <div className="mr-2">{coinIcon()}</div>
        {name}
      </div>
      <p>
        {name === 'Bitcoin' || name === 'Ethereum' || name === 'Tether' || true ? (
          <span className={style.byuButton} onClick={() => openModal()}>
            Buy
          </span>
        ) : (
          <></>
